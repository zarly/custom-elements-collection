...
> Looking for the per-component CSS-variable list? Each component declares the tokens it consumes in its meta file. Run `node scripts/components.mjs --tag ce-card --fields tag,cssVariables` (or open `src/components/card/card.meta.json`) for the precise list.
